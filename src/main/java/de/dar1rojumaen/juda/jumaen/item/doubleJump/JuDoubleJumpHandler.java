...
public class JuDoubleJumpHandler {
    ...
    private static void onClientTick(MinecraftClient client) {
        ...
        // Get the RegistryEntry for the Cloud Jump enchantment
        RegistryEntry<Enchantment> cloudJumpEntry = player.getWorld()
                .getRegistryManager()
                .get(RegistryKeys.ENCHANTMENT)
                .getEntry(Identifier.of(JuDaMod.MOD_ID, "cloud_jump")) // Replace "modid" with your namespace
                .orElse(null); // Handle missing entry gracefully

        int cloudLevel = 0;

        if (!boots.isEmpty() && cloudJumpEntry != null) {
            // Use EnchantmentHelper to get the level using the RegistryEntry
            cloudLevel = EnchantmentHelper.getLevel(cloudJumpEntry, boots);
            hasCloudJumpEnchantment = cloudLevel > 0;
        }
